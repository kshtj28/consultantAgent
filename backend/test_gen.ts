import { generateMultiSMEConsolidation } from './src/services/multiSMEConsolidationService';

async function test() {
    try {
        console.log("Testing generation for order_to_cash...");
        const result = await generateMultiSMEConsolidation({ processId: 'order_to_cash' });
        if (result == null) {
